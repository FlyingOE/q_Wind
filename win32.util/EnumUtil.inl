...
std::vector<std::string> util::EnumBase<C, E>::getAllStrings() {
...
template <typename C, typename E>
E util::EnumBase<C, E>::fromString(char const* str) {
	assert(NULL != str);
...
template <typename C, typename E>
bool util::EnumBase<C, E>::isValidString(char const* str) {
	assert(NULL != str);
	return getMap().find(str) != getMap().cend();
}

template <typename C, typename E>
void util::EnumBase<C, E>::registerEnumString(E const e, char const* str) {
	auto const result = getMap().insert(typename str2enum::value_type(str, e));
	assert(result.second);	//insertion must succeed!
}

template <typename C, typename E>
typename util::EnumBase<C, E>::str2enum& util::EnumBase<C, E>::getMap() {
	static str2enum map;
